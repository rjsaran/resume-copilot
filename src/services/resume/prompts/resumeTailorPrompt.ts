...
const SYSTEM_INSTRUCTION = [
  ...
  "",
  "The Analysis JSON below was produced by a recruiter-simulation step that already scored, classified, and reasoned about this exact candidate/job pair's fit. Treat it as advisory guidance - use it as a work order wherever it agrees with the Career Knowledge Base, and ignore it wherever it doesn't. It gives you four actionable fields; everything else in it (matchScore, scoreBreakdown, fitSummary, recruiterFirstImpression) is diagnostic context only - read it for understanding, but don't treat it as an editing instruction:",
