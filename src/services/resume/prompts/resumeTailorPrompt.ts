...
const SYSTEM_INSTRUCTION = [
  ...
  "",
  "basics.location, basics.linkedin, basics.github, and basics.portfolio are contact info, not content to select or trim: copy each one over exactly as it appears in the Base Resume whenever the Base Resume has it populated. Omitting one because it doesn't seem relevant to this job, or because you're trying to save space, is not a valid reason to drop it - contact info is never part of the ~2-page content budget.",
  "",
  "Group skills into the schema's fixed categories - languages, frameworks, cloud, databases, tools, other - putting each technology under whichever fits best (e.g. messaging/observability/DevOps tools belong under \"tools\", not a category of their own). Never invent a new category name; it has no field to go in and will be silently dropped from the resume. Avoid listing a technology more than once across categories. The Base Resume's own skills object is itself already a curated, ground-truth summary of the candidate's abilities - independent of which experience/project entries you choose to keep. Carry forward every skill relevant to this job from it, even if you end up dropping the specific experience entry that originally demonstrated it; never drop an entire skills category (e.g. databases) just because the entries mentioning those technologies didn't make the cut.",
  "",
  ...
  "- Drop basics.location, basics.linkedin, basics.github, or basics.portfolio, or any skills category, that the Base Resume has populated",
  "",
  "Output requirements:",
  ...
  "- Skills are grouped logically, and every skills category populated in the Base Resume is still present.",
  "- No duplicate technologies exist.",
  "- basics.location, basics.linkedin, basics.github, and basics.portfolio were all copied over if the Base Resume has them.",
