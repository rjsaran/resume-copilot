...
const STRENGTH_SCHEMA = {
  ...
  properties: {
    title: {
      ...
      maxLength: 40,
      description:
        'Short positive tag (e.g. "Payments domain", "Distributed systems").',
    },
    evidence: {
      type: "array",
...
const COVERAGE_ITEM_SCHEMA = {
  ...
  properties: {
    requirement: {
      ...
      maxLength: 40,
      description:
        'A single requirement pulled from the job description, name only (e.g. "Kafka", "PCI DSS").',
    },
    ...
    category: {
      ...
      description:
        'Only set for status "partial" or "missing" - omit entirely for "covered".',
    },
    severity: {
      type: "string",
...
const SCORE_BREAKDOWN_SCHEMA = {
  ...
  properties: {
    domain: {
      type: "integer",
      description:
        "0-100: how closely the candidate's industry/problem domain matches this role.",
    },
    responsibilities: {
      type: "integer",
      description:
        "0-100: how closely day-to-day responsibilities overlap with what this role actually does.",
    },
    seniority: {
      type: "integer",
      description:
        "0-100: whether the candidate's level (years, scope, ownership) matches what this role expects.",
    },
    technology: {
      type: "integer",
      description:
...
export const JOB_ANALYSIS_SCHEMA = {
  ...
  properties: {
    company: {
      type: "string",
      description:
        "The hiring company's name, extracted from the job description.",
    },
    jobTitle: {
      type: "string",
...
function isApplicationRecommendation(
  value: unknown,
): value is ApplicationRecommendation {
  if (typeof value !== "object" || value === null) return false;
  const v = value as Record<string, unknown>;
  return (
    (v.decision === "Apply" ||
