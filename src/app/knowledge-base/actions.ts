...
export async function saveKnowledgeBaseAction(
  ...
  const log = logger.child({
    action: "saveKnowledgeBaseAction",
    userId: user.id,
  });

  if (!isCareerKnowledgeBase(data)) {
    log.warn(
      "Knowledge base save blocked: payload did not match expected schema",
    );
    return {
      success: false,
      error: "Knowledge base did not match the expected schema.",
    };
  }
...
export async function importKnowledgeBaseAction(
  ...
  const user = await requireUser();
  const log = logger.child({
    action: "importKnowledgeBaseAction",
    userId: user.id,
  });
  ...
  if (
    ...
    log.warn("Import blocked: uploaded file is not a PDF", {
      fileType: file.type,
    });
  ...
  if (sourceText.length > MAX_IMPORT_TEXT_LENGTH) {
    log.warn("Import blocked: extracted text too long", {
  ...
  try {
    const knowledgeBase = await importKnowledgeBaseFromText(
      { sourceText },
      provider,
    );
    ...
    log.error("Knowledge base import failed", {
      ...errorContext(error),
      durationMs: Date.now() - startedAt,
    });
    return {
